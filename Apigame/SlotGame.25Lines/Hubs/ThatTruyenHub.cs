...
{
    [HubName("SingleWild25lines")]
    public class ThatTruyenHub : Hub
    {
        ...
        public async Task PlayNow(MoneyType moneyType, int roomId)
        {
            ...
            try
            {
                ...
                var accountInfo = ThatTruyenGameHandler.Instance.GetAccountInfo(accountId, accountName, roomId, moneyType, out var response);
                if (response > 0)
                {
        ...
        public void Spin(MoneyType moneyType, int roomId, string lines)
        {
            ...
            try
            {
                ...
                if(accountId < 1)
                    return;
                var spinResult = ThatTruyenGameHandler.Instance.Spin(accountId, accountName, roomId, moneyType, lines);
        ...
        public void PlayLuckyGame(MoneyType moneyType, X2Game step, int roomId, int spinId)
        {
            ...
            try
            {
                ...
                var gameResult = ThatTruyenGameHandler.Instance.PlayLuckyGame(moneyType, accountId, accountName, roomId, step, spinId);
        ...
        public void PlayBonusGame(MoneyType moneyType, int spinId)
        {
            try
            {
                ...
                long response = ThatTruyenGameHandler.Instance.FinishBonusGame(moneyType, spinId, out var prizeValue, out var balance);
        ...
        {
            Clients.Caller.UpdateJackpot(ThatTruyenGameHandler.Instance.GetJackpot(MoneyType.Gold));
            var accountId = AccountSession.AccountID;
            var accountName = AccountSession.AccountName;
            if (accountId > 0)
            {
